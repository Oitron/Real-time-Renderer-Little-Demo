...
struct Camera
{
	...
	void consumeOrbit(float _amount)
	{
		...
		const float invToPosLen  = 1.0f / (toPosLen + bx::kFloatMin);//avoid dividing by 0
		const bx::Vec3 toPosNorm = bx::mul(toPos, invToPosLen);

		float ll[2];
		bx::toLatLong(&ll[0], &ll[1], toPosNorm);//to latitude and longitude
		ll[0] += consume[0];
		ll[1] -= consume[1];
		ll[1]  = bx::clamp(ll[1], 0.02f, 0.98f);//avoid flip
