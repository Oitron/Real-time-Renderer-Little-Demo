...
vec3 specular_brdf(vec3 f0, float roughness, float metallic, vec3 lightDir, vec3 viewDir, vec3 normal){
    ...
    return fresnel(f0, halfWayDir, viewDir) *
           ...
           (4.0 * max(dot(normal, lightDir),0.0) * max(dot(normal, viewDir), 0.0) + F_MIN) * metallic;
}
...
void main(){
    ...
    vec3 albedo = u_objColor.xyz;

    float roughness = u_objRM.x;
    float metallic = u_objRM.y;
    ...
    vec3 direct_lighting = (diffuse_brdf(albedo, metallic) 
                          + specular_brdf(f0, roughness, metallic, lightDir, viewDir, v_normal)) 
                          * u_lightColor.xyz * max(dot(v_normal, lightDir), 0.0);
