...
void main(){
    ...
    vec3 objColor = toLinear(texture2D(s_texColor, v_texcoord0)).xyz; //sRGB -> linear

    //---shadow---//
    float visible = Basic_ShadowCalculation(v_fragPosLightSpace, lightDir);
    //vec3 objColor = texture2D(s_texColor, v_texcoord0).xyz;
    vec3 lighting = attenuation * blinn_phong(tsLightDir, tsViewDir, tsNormal, shiniess, objColor, visible);
